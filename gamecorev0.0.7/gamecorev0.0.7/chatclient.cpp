...
unsigned char chatclient::GetPacketIdentifier(RakNet::Packet *p)
{
	if (p==0)
		return 255;
...
void chatclient::runchatclient()
{
	...
	// Holds user data
	char ip[64]="192.168.1.7", serverPort[30]="1080", clientPort[30]="80";

	// A client
	isServer=false;

	//printf("Game Client v0.0.1\n\n");
	...
	//if (ip[0]==0)
	//	strcpy(ip, "108.57.87.165");

	//puts("Enter the port to connect to");
	//printf("->");
	//Gets(serverPort,sizeof(serverPort));
	//if (serverPort[0]==0)
	//	strcpy(serverPort, "1080");
	...
	//puts("Enter username");//get a chatname
	//printf("->");
	//Gets(useri,sizeof(useri));
	//strcpy (pfx,": ");
	//strncat(useri,pfx, (sizeof(useri)+ sizeof(pfx)) );
...
#else
	RakNet::ConnectionAttemptResult car = client->Connect(ip, atoi(serverPort), "0", (int) strlen("0"));
	...
	while (1)
	{
		...
		if (kbhit())
		{
			...
			if (strcmp(message, "connect")==0)
			{
				...
				bool b = client->Connect(ip, atoi(serverPort), "Rumpelstiltskin", (int) strlen("Rumpelstiltskin"), &pk)==RakNet::CONNECTION_ATTEMPT_STARTED;	
#else
				bool b = client->Connect(ip, atoi(serverPort), "0", (int) strlen("0"))==RakNet::CONNECTION_ATTEMPT_STARTED;	
#endif
			...
			strncpy(prefix, useri,sizeof(useri));

			strncpy(message2, prefix, sizeof(message2));
			strncat(message2, message, sizeof(message2) - strlen(prefix) - 1);

			client->Send(message2, (const int) strlen(message2)+1, HIGH_PRIORITY, RELIABLE_ORDERED, 0, RakNet::UNASSIGNED_SYSTEM_ADDRESS, true);
		}
