...
void IC_Console::initializeConsole(irr::gui::IGUIEnvironment* guienv, const irr::core::dimension2d<s32>& screenSize)
{
	...
	appendMessage(L"#############################################");
	appendMessage(L"Gamecore v0.1.1 Console ");
...
void IC_Console::loadDefaultCommands(irr::IrrlichtDevice* device)
{
	...
	cmd = new IC_Command_TP (thePlayer_);
	registerCommand(cmd);

	cmd = new IC_Command_RB (thePlayer_,world_,device);
	registerCommand(cmd);
	
	cmd = new IC_Command_GV(world_);
	registerCommand(cmd);
	
	cmd = new IC_Command_PGV(thePlayer_);
	registerCommand(cmd);
	...
	cmd = new IC_Command_C(thePlayer_);
	registerCommand(cmd);

	cmd = new IC_Command_H(thePlayer_);
	registerCommand(cmd);
...
void IC_Console::handleKeyPress(wchar_t keyChar, irr::EKEY_CODE keyCode, bool bShiftDown, bool bControlDown)
{
	...
	else if(keyCode == irr::KEY_UP || keyCode == irr::KEY_LEFT)
	{
		if(consoleHistory.size() > 0)
		{
			...
			u32 index = consoleHistory.size() - 1 - consoleHistoryIndex;
	...
	else if(keyCode == irr::KEY_DOWN || keyCode == irr::KEY_RIGHT)
	{
		if(consoleHistory.size() > 0)
		{
			...
			u32 index = consoleHistory.size() - consoleHistoryIndex;
			if(index >= 0 && index < consoleHistory.size())
			{
				consoleHistoryIndex--;
				currentCommand = consoleHistory[index];
...
WideString& IC_Console::handleCommandString(WideString& wstr)
{
