...
InternalServer::InternalServer(bool online,irr::IrrlichtDevice* device,char* filename,
							   ...
							   ,int remote_port, int local_port,ChatQueue* message_buffer): online_(online),
device_(device),smgr_(smgr),driver_(driver), engine_(engine), world_(world),networkData_(device,filename,smgr,driver,engine,world, message_buffer)
	...
	if(online_)
	{
			...
			for (p=client_->Receive(); p; client_->DeallocatePacket(p), p = client_->Receive())
			{
				...
				default:// It's a client
					...
					if(p->data[0]!= '0' && p->data[0]!= '9' ){//if the packet id is 0 or 99 do not print it
						printf("%s", p->data);
						puts("\n");
